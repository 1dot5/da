...
| 配置 | 用途 |
|---|---|
| `app/{route}/_components/` | そのページでしか使わないコンポーネント |
| `packages/` | アプリ横断の共通 UI（2 アプリ以上で使うもの） |
| `hooks/` | アプリ全体で使うカスタムフック |
| `store/` | グローバルな UI 状態（Zustand） |
| `api/` | API クライアント関数・GraphQL 定義 |
| `utils/` | 副作用のないユーティリティ |
...
| ケース | 方法 |
|---|---|
| 初回表示で決まるデータ | Server Component |
| 書き込み頻度が高く操作後に更新が必要 | TanStack Query |
| 短時間で画面間共有する | TanStack Query（QueryCache） |
| マスターデータ（変更頻度低） | Server Component + `cache` / `'use cache'` |
...
| URL に乗る状態（検索条件・ページ番号・タブ） | `useSearchParams` + `useSearchParamsState` 的な hook |
...
- `unstable_cache` からの移行は [Vercel 公式ガイド](https://vercel.com/docs) 参照
...
- エラーは `aria-describedby` で関連付け（[.agent/review.md](review.md)）
